...
class SynqNearbyModule: Module() {
 ...
 private fun startBle(){val a=adapter();if(!a.isEnabled){sendEvent("onState",mapOf("state" to "bluetooth-unavailable"));return};val data=token.toByteArray();advertiseCallback=object:AdvertiseCallback(){override fun onStartFailure(code:Int){sendEvent("onState",mapOf("state" to "advertise-failed","code" to code))}};a.bluetoothLeAdvertiser?.startAdvertising(AdvertiseSettings.Builder().setAdvertiseMode(AdvertiseSettings.ADVERTISE_MODE_LOW_POWER).setConnectable(false).build(),AdvertiseData.Builder().addServiceUuid(uuid).addServiceData(uuid,data).build(),advertiseCallback);callback=object:ScanCallback(){override fun onScanResult(t:Int,r:ScanResult){val found=r.scanRecord?.getServiceData(uuid)?.toString(Charsets.UTF_8)?:return;if(found==token)return;sendEvent("onEncounter",mapOf("epochToken" to found,"rssiBucket" to if(r.rssi>-65)"likely-near" else "uncertain","observedAt" to System.currentTimeMillis()))}};a.bluetoothLeScanner?.startScan(listOf(ScanFilter.Builder().setServiceUuid(uuid).build()),ScanSettings.Builder().setScanMode(ScanSettings.SCAN_MODE_LOW_POWER).build(),callback)}
